...
def _strip_answer_markers(stem: str) -> str:
    ...
    filtered = [part for part in parts if part not in ANSWER_MARKERS]
    text = "-".join(filtered)
    for marker in ("答案", "解析"):
...
def discover_workspace(root: Path) -> WorkspaceDiscoveryResult:
    ...
    unmatched: list[ProblemFile] = []
    ambiguous: list[AmbiguousAnswerMatch] = []
    answers = [AnswerKeyFile(path=path) for path in answer_paths]
    ...
    for problem_path in problem_paths:
        ...
        problem = ProblemFile(path=problem_path)
        if len(candidates) == 1:
            pairs.append(ProblemAnswerPair(problem=problem, answer=candidates[0]))
        elif len(candidates) > 1:
            ambiguous.append(AmbiguousAnswerMatch(problem=problem, candidates=candidates))
        else:
